"""
Explanation:
ftvq Function:
Initializes with the provided image and precomputed parameters.
Performs padding to handle edge effects during deconvolution.
Computes initial estimates of the deconvolved image using the provided blur kernels.
Iteratively refines the estimate using gradient descent and shrinkage.
precomp Function:
Computes precomputed values used in the iterative update process.
padimg Function:
Adds padding to the image to handle boundary effects during convolution.
gets Function:
Computes the shrinkage parameters based on the current image gradients and regularization weight.
shrinkv23 Function:
Computes the shrinkage function based on a quartic equation and the given regularization weight.
Dependencies:
NumPy: For numerical operations.
SciPy: For FFT operations.
OpenCV (cv2): For disk-shaped filters. You can install it with pip install opencv-python.
Make sure to adapt and test the code with your data and environment,
"""

import numpy as np
from scipy.fft import fft2, ifft2
from scipy.ndimage import convolve
import cv2  # For disk-shaped filter

def ftvq(y, pc, mu):
    out_it = 8
    beta_s = 4
    beta = (100 * mu) / (beta_s ** out_it)

    # Padding the image
    pad = pc[0]
    y_padded = padimg(y, pad)

    # Precompute necessary quantities
    num2, den1, den2 = precomp(y_padded, pc)

    # Initialization
    x = np.copy(y_padded)
    fct = 0.5
    for i in range(3):
        x[..., i] = np.real(ifft2(fct * num2[..., i] / (den1 + fct * den2[..., i])))

    # Find gradient directions
    w0x, w0y = np.gradient(x, axis=(1, 0))
    w0n = np.sqrt(np.sum(w0x ** 2, axis=2, keepdims=True))
    w0x = np.where(w0n == 0, 1 / np.sqrt(3), w0x / w0n)
    w0n = np.sqrt(np.sum(w0y ** 2, axis=2, keepdims=True))
    w0y = np.where(w0n == 0, 1 / np.sqrt(3), w0y / w0n)

    for _ in range(out_it):
        # W sub-problem
        wx = np.diff(x, axis=1) / np.sqrt(2)
        wy = np.diff(x, axis=0) / np.sqrt(2)

        # Project
        wxj = np.sum(wx * w0x, axis=2)
        wyj = np.sum(wy * w0y, axis=2)

        # Shrinkage
        mx = max(1e-4, np.max(np.abs(wxj)), np.max(np.abs(wyj)))
        thr, a, b = gets(beta, mx)

        wxj = np.where(np.abs(wxj) < thr, 0, np.maximum(0, a * np.abs(wxj) + b) * np.sign(wxj))
        wyj = np.where(np.abs(wyj) < thr, 0, np.maximum(0, a * np.abs(wyj) + b) * np.sign(wyj))

        # Final gradient vectors
        wx = wxj[..., np.newaxis] * w0x
        wy = wyj[..., np.newaxis] * w0y

        # X sub-problem
        for i in range(3):
            num1 = np.concatenate([
                -wx[:, 0, i:i+1], -np.diff(wx[..., i:i+1], axis=1), wx[:, -1, i:i+1]], axis=1
            ) + np.concatenate([
                -wy[0, :, i:i+1], -np.diff(wy[..., i:i+1], axis=0), wy[-1, :, i:i+1]], axis=0
            )
            tmp = fft2(num1 / np.sqrt(2)) + mu / beta * num2[..., i]
            tmp = tmp / (den1 + mu / beta * den2[..., i])
            x[..., i] = np.real(ifft2(tmp))

        beta *= beta_s  # Beta continuation

    # Remove padding
    x = x[pad:-pad, pad:-pad, :]
    return x

def precomp(y, pc):
    den1 = pc[5]
    den2 = pc[6]
    k1f, k2f, k3f = pc[1], pc[2], pc[3]

    num2 = np.stack([np.conj(k1f) * fft2(y[..., 0]),
                     np.conj(k2f) * fft2(y[..., 1]),
                     np.conj(k3f) * fft2(y[..., 2])], axis=-1)
    return num2, den1, den2

def padimg(y, pad):
    y2 = np.zeros((y.shape[0] + 2 * pad, y.shape[1] + 2 * pad, y.shape[2]))
    wt = np.linspace(0, 1, 2 * pad)
    wtx = np.tile(wt[np.newaxis, :], (y.shape[0], 1))
    wty = np.tile(wt[:, np.newaxis], (1, y2.shape[1]))
    for i in range(3):
        yi = y[..., i]
        px = (1 - wtx) * yi[:, -1][:, np.newaxis] + wtx * yi[:, 0][:, np.newaxis]
        yi = np.hstack([px[:, pad:], yi, px[:, :pad]])
        py = (1 - wty) * yi[-1, :] + wty * yi[0, :]
        yi = np.vstack([py[pad:, :], yi, py[:pad, :]])
        y2[..., i] = yi
    return y2

def gets(beta, mx):
    v = np.linspace(0, mx, 5000)
    v2 = shrinkv23(v, beta)
    idx = np.min(np.where(v2 > 0))

    if np.isnan(idx):
        return 0, 0, 10

    thr = v[idx]
    a = (v2[-1] - v2[idx]) / (v[-1] - v[idx])
    b = v2[-1] - a * v[-1]
    return thr, a, b

def shrinkv23(v, beta):
    epsilon = 1e-6

    k = 8 / (27 * beta**3)
    m = k * np.ones_like(v)

    v2 = v ** 2
    v3 = v2 * v
    v4 = v3 * v
    m2 = m ** 2
    m3 = m2 * m

    alpha = -1.125 * v2
    beta2 = 0.25 * v3

    q = -0.125 * m * v2
    r1 = -q / 2 + np.sqrt(-m3 / 27 + (m2 * v4) / 256)

    u = np.exp(np.log(r1) / 3)
    y = 2 * (-5 / 18 * alpha + u + (m / (3 * u)))

    W = np.sqrt(alpha / 3 + y)

    root = np.zeros((v.shape[0], v.shape[1], 4))
    root[:, :, 0] = 0.75 * v + 0.5 * (W + np.sqrt(-(alpha + y + beta2 / W)))
    root[:, :, 1] = 0.75 * v + 0.5 * (W - np.sqrt(-(alpha + y + beta2 / W)))
    root[:, :, 2] = 0.75 * v + 0.5 * (-W + np.sqrt(-(alpha + y - beta2 / W)))
    root[:, :, 3] = 0.75 * v + 0.5 * (-W - np.sqrt(-(alpha + y - beta2 / W)))

    v2_expanded = np.repeat(v[:, :, np.newaxis], 4, axis=2)
    sv2 = np.sign(v2_expanded)
    rsv2 = np.real(root) * sv2

    valid_root = ((np.abs(np.imag(root)) < epsilon) &
                  (rsv2 > np.abs(v2_expanded) / 2) &
                  (rsv2 < np.abs(v2_expanded)))
    root_flag3 = np.sort(valid_root * rsv2, axis=2)[:, :, ::-1] * sv2

    w = root_flag3[:, :, 0]
    return w


